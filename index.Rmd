...
```{r}
get_tidy_audio_analysis("6IUhPMJf4iJQ3Go1CkHDsa") %>%
...
```{r}
get_tidy_audio_analysis("6bvTzuFABmFnAAmVZ3nlMh") %>%
  compmus_align(bars, segments) %>%
  select(bars) %>%
  unnest(bars) %>%
  mutate(
  ...
  labs(x = "Time (s)", y = "")
```
Comparison of a atypical track (*Los*) to a more typical track (*Reise, Reise*) from Rammstein, both from the album *Reise Reise*.
